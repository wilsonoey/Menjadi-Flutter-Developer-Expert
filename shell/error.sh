...
extractFailedTests() {
  ...
  awk '
    /\[E\]/ {
      if (NR > 1) print "================================================================================" >> "'$outputFile'"
      print $0 >> "'$outputFile'"
      
      # Print next lines that contain error details
      for (i = 1; i <= 10; i++) {
        if (getline > 0) {
          if ($0 ~ /^[0-9]{2}:[0-9]{2}/) break
          print $0 >> "'$outputFile'"
        }
      }
    }
...
*)
  ...
  extractFailedTests "$currentDir/$LOG_FILE" "$currentDir/$FAILED_LOG_FILE"
  echo "Failed tests summary saved to: $FAILED_LOG_FILE"
  echo ""
  cat "$currentDir/$FAILED_LOG_FILE"
